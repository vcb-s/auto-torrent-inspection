namespace NChardet
{
    ...
    {
        internal static readonly byte eStart       = 0;
        internal static readonly byte eError       = 1;
        internal static readonly byte eItsMe       = 2;
        ...
        public abstract bool isUCS2();
         ...
         public static byte getNextState(Verifier v, byte b, byte s)
         {
             return (byte)
                     (0xFF & (v.states()[
                         ((s*v.stFactor() +
                           ((v.cclass()[(b & 0xFF) >> eidxSft4bits] >> ((b & eSftMsk4bits) << eBitSft4bits)) &
                            eUnitMsk4bits)) & 0xFF) >> eidxSft4bits] >>
                              (((s*v.stFactor() +
                                 ((v.cclass()[(b & 0xFF) >> eidxSft4bits] >> ((b & eSftMsk4bits) << eBitSft4bits)) &
                                  eUnitMsk4bits)) & 0xFF & eSftMsk4bits) << eBitSft4bits)) & eUnitMsk4bits);
         }
    }
}
