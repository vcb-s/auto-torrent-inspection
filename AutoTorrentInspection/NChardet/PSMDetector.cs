    ...
    public abstract class PSMDetector
    {
        ...
        public bool HandleData(byte[] aBuf, int len)
        {
            for (int i = 0; i < len; i++)
            {
                ...
                {
                    var st = Verifier.getNextState(mVerifier[mItemIdx[j]], b, mState[j]);
                    ...
                    if (st == Verifier.eError)
                    {
                        mItems--;
                        if (j >= mItems) continue;
        ...
        public void Sample(byte[] aBuf, int aLen, bool aLastChance)
        {
            ...
            if (!mRunSampler) return;
            mRunSampler = mSampler.Sample(aBuf, aLen);
            if (((!aLastChance || !mSampler.GetSomeData()) && !mSampler.EnoughData()) || (eucNum != possibleCandidateNum)) return;
            mSampler.CalFreq();
