namespace NChardet
{
    public class EUCSampler
    {
        ...
        public void CalFreq()
        {
            ...
            for (int i = 0; i < 94; i++)
            {
                mFirstByteFreq[i]  = mFirstByteCnt[i]  / total;
                mSecondByteFreq[i] = mSecondByteCnt[i] / total;
            }
        }
        ...
        public static float GetScore(float[] array1, float[] array2)
        {
            float sum = 0.0f;
            for (int i = 0; i < 94; i++)
            {
            ...
            return (float)System.Math.Sqrt(sum) / 94.0f;
        }
    }
}
